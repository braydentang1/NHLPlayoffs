...
add_knn_variables <- function(train_data, test_data, include_PCA = FALSE, distances = TRUE, use_only_variables =  NULL) {
  ...
  if (distances == TRUE) {
    ...
    knn_test <- new_frames_with_knn$new.te %>% 
      as_tibble(.) %>%
...
model_pipe_outer <- function(subset_var, lambda_final, alpha_final, times, processed_data, variable_importance) {
  ...
  train <- processed_data$train %>%
    select(variable_importance$variable, result_factor)
...
process_folds <- function(fold_index, main_train, use_only_variables = NULL) {
  ...
  #' Given a set of indices representing the training dataset (relative to the dataset main_train),
  #' process the training and implied testing data sets. 
  #'
  #' @param fold_index a vector of indices corresponding to the training dataset (relative to main_train). 
...
process_var_imp <- function(var_imp_raw) {
  ...
  #'
  #' @param var_imp_raw a list of tibbles that contain variable importance scores produced from the function baggedModel.
  #'
  #' @return 
...
rfe_selection <- function(all_processed_frames, subset_size, var_imp, times = 20, n_iters = 25) {
  
  #' Trains a model with a reduced subset of features as given by subset.size.
  #'
  #' @param all_processed_frames a list that has two named tibbles; train and test. Should be the result of calling the function process_folds.
  #' @param subset_size an integer value that takes the top (subset_size) number of features, as given in the var_imp argument
  #' @param var_imp a tibble that has two columns: variable, which gives the variable name corresponding to a column in any of the tibbles in
  #'  all_processed_frames, and importance, which provides the variable importance from the bagged elastic net using all features.
  #'
  #' @return
  #' tibble
  ...
  best_param <- BayesianOptimization(FUN =  function(alpha, lambda) {
    ...
    for (m in 1:length(all_processed_frames)) {
      ...
      train <- all_processed_frames[[m]]$train %>% select(var_imp_sorted$variable, result_factor)
      test <- all_processed_frames[[m]]$test %>% select(var_imp_sorted$variable, result_factor)
      
      model <- bagged_model(
        train = train, 
        test = test, label_train = train$result_factor, 
        alpha = alpha,
        s_lambda = as.integer(lambda), 
        times = times, 
        calibrate = FALSE)
      
      scores[m] <- log_loss(scores = model$predictions, label = test$result_factor)
  ...
  initPoints = 3, nIters = n_iters, convThresh = 1e+02, verbose = 0)
  
  tibble(subset_size = subset_size,
         alpha = best_param$ScoreDT$alpha[which.max(best_param$ScoreDT$Score)], 
