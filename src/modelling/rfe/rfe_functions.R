...
rfe_selection <- function(all_processed_frames, subset_size, var_imp, times = 20, n_iters = 25) {
  ...
  #'
  #' @param allProcessedFrames a list that has two named tibbles; Train and Test. Should be the result of calling the function processFolds.
  #' @param subset.size an integer value that takes the top (subset.size) number of features, as given in the varImp argument
  #' @param varImp a tibble that has two columns: Variable, which gives the variable name corresponding to a column in any of the tibbles in allProcessedFrames, and Importance, which provides the variable importance from the bagged elastic net using all features.
  #'
  #' @return
  #' tibble
