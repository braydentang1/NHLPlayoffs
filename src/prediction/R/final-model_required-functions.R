...
bagged_model <- function(train, test, label_train, alpha, s_lambda, times, calibrate = FALSE) {
  ...
  #' @param alpha a numeric value in [0,1] to train each elastic net model with in the bag.
...
train_model_one_rep <- function(rep_index, inner_folds, main_train, times = 20, n_iters = 25) {
  ...
  #' Trains one repetition of the bagged elastic net model, to be used in an ensemble. Uses a list created by calling
  #'  caret::createResamples
  #'
  #' @param rep_index A numeric vector of length one containing the specific rep to use for training in inner_folds.
  #' @param inner_folds A list of repeated k-folds.
  #' @param main_train A tibble representing the training dataset.
  #' @param times An integer representing the number of bootstrap resamples to use in the bagged elastic net model.
  #' @param n_iters An integer representing the number of Bayesian optimization iterations to use for hyperparameter searching.
  #'
  #' @return A tibble with one row, containing the best alpha and lambda found for the rep_index repeat given in inner_folds.
  #' @export
  #'
