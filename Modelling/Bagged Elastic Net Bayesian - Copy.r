...
rfeSelection = function(allProcessedFrames, subset.size, varImp){
  ...
  initPoints = 3, nIters = 5, verbose = 2)
...
giveResults = function(seed, allData){
  ...
  rfeResults = bind_rows(lapply(c(20,50,80), FUN = rfeSelection, allProcessedFrames = allProcessedFrames, varImp = finalVarImp)) %>%
    bind_rows(tibble(Subset.Size = Inf, alpha = processed.bestParam$alpha[1], lambda = as.integer(processed.bestParam$lambda[1])))
  
  processedData = processFolds(fold.index = allFolds[[1]], mainTrain = allData)
  finalTestSet.Score = mapply(modelPipe.outer, lambda.final = rfeResults$lambda, alpha.final = rfeResults$alpha, MoreArgs = list(processedData = processedData), SIMPLIFY = FALSE) %>%
...
clusterEvalQ(cluster, c(library(caret), library(forecast), library(tidyverse), source("C:/Users/Brayden/Documents/GitHub/NHLPlayoffs/Modelling/All Functions - Copy.R")))
results = parLapply(NULL, allSeeds, fun = giveResults, allData = allData)

stopCluster(cluster)
rm(cluster)

processedResults = results %>% reduce(., left_join, by = "Subset.Size")
finalLogLoss = select(-Subset.Size) %>% transmute(Overall = rowMeans(.))
