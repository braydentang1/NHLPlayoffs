...
modelPipe.inner = function(folds, seed.a, iterations, allData){
  ...
  allProcessedFrames = lapply(innerFolds, mainTrain = mainTrain, FUN = processFolds)
  
  for(m in 1:length(innerFolds)){
  
    results[[m]] = randomGridSearch(innerTrainX = allProcessedFrames[[m]]$Train, innerTestX = allProcessedFrames[[m]]$Test, grid = grid)
