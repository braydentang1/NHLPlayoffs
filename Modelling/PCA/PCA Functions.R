...
PCA_recipe = function(ncomp, train, test){
  ...
  # Arguments:
  #
  # ncomp -- an integer specifying the number of components to retain
  # train -- a tibble of the training dataset. Should contain the target variable (ResultProper) as a factor variable, but all predictors/features should be numeric.
  # test -- a tible of the testing dataset. Should also contain the target variable (ResultProper) as a factor variable, and only contain numeric predictors.
  # Returns:
  #
  # list
...
processFolds = function(fold.index, mainTrain, useOnlyVariables = NULL){
  ...
  frameswithKNN = addKNN_variables(traindata = train, testdata = test, distances = TRUE, include_PCA = FALSE, useOnlyVariables = useOnlyVariables)
